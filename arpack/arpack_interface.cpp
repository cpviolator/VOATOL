...
int main(int argc, char **argv) {
  ...
  if (argc < 8 || argc > 8) {
    ...
    cout << "./arpack <nKr> <nEv> <max-restarts> <diag> <tol> <hermitian> <eigen_comp>" << endl;
    cout << "./arpack 60 20 200 100 1e-12 1 0 " << endl;
    exit(0);
  }
  ...
  bool eigen_comp = atoi(argv[7]) == 1 ? true : false;
  ...
  if(eigen_comp) {
    ...
    for(int i=0; i<Nvec; i++) cout << eigensolverRef.eigenvalues()[i] << " " << abs(eigensolverRef.eigenvalues()[i]) << endl;
  ...
  for(int i=0; i<nev_; i++){    
    printf("EigVal[%04d]: ||(%+.16e, %+.16e)|| = %.16e residual = %.16e\n", i,
  ...
  for (int i = 0; i < nEv && eigen_comp; i++) {
    ...
    if(hermitian) {
      ...
      printf("EigenComp[%04d]: [(%+.8e, %+.8e) - (%+.8e, %+.8e)]/(%+.8e, %+.8e) = (%+.8e,%+.8e)\n", i, evals_[idx].real(), evals_[idx].imag(), eigensolverRef.eigenvalues()[idx_e].real(), eigensolverRef.eigenvalues()[idx_e].imag(), eigensolverRef.eigenvalues()[idx_e].real(), eigensolverRef.eigenvalues()[idx_e].imag(), (evals_[idx].real() - eigensolverRef.eigenvalues()[idx_e].real())/eigensolverRef.eigenvalues()[idx_e].real(), (evals_[idx].imag() - eigensolverRef.eigenvalues()[idx_e].imag())/eigensolverRef.eigenvalues()[idx_e].imag());
    }
  }
